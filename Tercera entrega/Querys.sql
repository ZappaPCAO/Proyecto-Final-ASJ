...
SELECT TOP 1 Providers.business_name, Providers.cod_provider,
        ISNULL(Providers.url_img, '-') AS 'Imagen', Providers.email, Providers.phone,
...
FROM Providers
    ...
    LEFT JOIN Contacts_Data ON (Providers.id = Contacts_Data.id_provider)
GROUP BY Providers.business_name, Providers.cod_provider, Providers.url_img, Providers.email, Providers.phone,
        Contacts_Data.name, Contacts_Data.last_name, Contacts_Data.email, Contacts_Data.phone, Contacts_Data.role


SELECT ISNULL(Providers.url_img, '-') AS 'Imagen' FROM Providers
